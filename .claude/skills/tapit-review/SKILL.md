...
Get enough context to judge the change correctly before reviewing it. These are the same lightweight principles `tapit-context` uses, inlined here rather than invoked, since Claude Code skills don't compose by calling one another:
...
5. If backend Pydantic schemas or frontend TS types are among the changed files, also run `tapit-ai review contracts` (from `tap-it-ai-tools/`, requires its own `.env` -- see that repo's `CLAUDE.md`) instead of re-deriving contract diffs by hand.
