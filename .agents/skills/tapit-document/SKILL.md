...
Manual-only: this skill runs only when invoked directly (`/tapit-document`) or explicitly requested. Codex does not trigger it automatically on its own judgment -- `disable-model-invocation` enforces that at the tool level, not just by instruction.
